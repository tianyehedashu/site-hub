---
name: 1688-sourcing
description: 1688 找品与 ziniao 预设（关键词搜、图搜、详情、店铺、落盘）；触发词：1688、以图搜款、关键词搜款、跨境找品、供应商初筛、跨境货源、site-hub 1688。
allowed-tools: Bash(ziniao:*)
---

# 1688 货源初筛（ziniao site-hub）

本 skill 面向跨境电商运营的 **1688 货源初筛**：用**关键词**或**图片**找候选、做粗略价格带判断、确认商品详情页是否有效、保存详情图样本，并记录店铺页的轻量信号。它不替代人工验厂、旺旺沟通、样品测试、价格谈判，也不声称能稳定获取销量、库存、MOQ、资质、发货时效或售后承诺。
...
- **关键词找款**：只有品类/品名描述，没有参考图，希望先拉一页候选 `offer_id` 再人工筛选。
...
- 须在 **1688 域已登录标签** 执行；若活动标签停在 **淘宝登录中转**（`login.taobao.com`），预设会返回 `wrong_tab`，请先 `ziniao navigate "https://www.1688.com/"` 完成登录再重试。
- 成功时除 `offers` 外还有 `offers_lite`（前 80 条摘要）；`offers` 元素通常含 `id`（即 `offer_id`），可与下方「校验候选详情」衔接。
- 与图搜返回结构**不完全相同**；排序、过滤、字段以 1688 当前接口为准，改版时需对照 `references/PRESETS.md`。
...
- **样本来源**：参考图或关键词（`keyword-search` / `image-search`）、抓取时间、登录会话是否正常。
