...
配图分三类，**它们在成品文章里出现的位置完全不同**，不可互相替代：

| 类型 | 抖音里叫什么 | 发布后在哪看到 | 本 skill 里的字段 | 规格要求 |
| --- | --- | --- | --- | --- |
| 正文头图 | 文章头图（addIcon[0]） | **文章详情页顶部**的大图 | `head_image` | 推荐 16:9 |
| 推荐封面 | 封面设置（addIcon[1]） | **推荐流/列表页**缩略图 | `cover_image` | **3:4 竖图、≥1080×1440**，否则被拒 |
| 段内图 | 正文里的 `<img>` | **文章详情页正文**（图文穿插） | manifest `blocks[*].img` → 走 tiptap `insertContent` | 无硬规格，`build_html.py` 统一 640×360/q=75 内联 base64 |

三条关键事实（踩过才记住）：

- **`head_image` 不会自动进正文。** 它只是顶部元数据，正文要有图必须靠 `blocks` 里明确的 `img` 条目——正文里空无一图，文章详情页的正文区就是纯文字（列表页看到缩略图 ≠ 正文有图）。
- **`cover_image` 只出现在列表。** 它不影响详情页外观，也不会被读者感知成"文章的配图"，只影响点击率。
- **段内图才是决定阅读体验的关键**；这一层图=0 的稿件等同于"长微博截图"，抖音留存会很差。

内容约束：

1. **正文头图（head）** 必须含**人物或动作或有情绪的物件**，不是纯风景/纯物件摆拍；画面要能被正文其中一段呼应（正文讲"把闹钟往后拨"，头图就出现床头/闹钟/窗帘）。
2. **推荐封面（cover）** 可以单独出一张 3:4 竖图（同 style prefix 换 `aspect ratio 3:4`），也可以直接复用 head 源图由 `build_html.py` 中心裁切——此时 head 源图的**长边必须 ≥1440**，否则脚本会打印 warning，上传时抖音会拒。
3. **段内图（body）** 每张图锚定**其所在段的核心名词或动作**，不重复、不装饰、不凑数。
...
3. **点进文章详情看成品**：**这一步不能省**。列表页看到的是 `cover_image`（3:4 缩略图），点进去才能看到 `head_image`（顶部大图）+ 正文 + 正文 `<img>`。若详情页**正文区没图**但管理页缩略图正常，说明 manifest 里 `blocks` 漏配了 `img` 条目——`head_image`/`cover_image` 都**不会**自动进正文。
4. **找不到怎么办**：
