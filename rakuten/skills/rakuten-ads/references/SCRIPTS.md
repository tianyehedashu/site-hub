...
路径：`rakuten-ads/`（`scripts/`、`references/`）。在 `ziniao rakuten … -o` 落盘后，用脚本做 JSON **合并**与**汇总**，口径与业务解读以 `SKILL.md` 为准。说明文档由 `SKILL.md` **直接**链入本文即可，**勿**再在 `references/` 下叠套多级子文档。
...
python scripts/aggregate_ad_exports.py -d $batch -o "$batch/summary.json"

python scripts/aggregate_ad_exports.py "$batch/rpp_daily.json" "$batch/rpp_item_merged.json" "$batch/tda_item.json" -o "$batch/summary.json"
```
...
python scripts/merge_rpp_search_json.py `
...
```powershell
python scripts/merge_cpnadv_report_json.py -o exports/cpnadv_all.json exports/p0.json exports/p1.json
```
...
scripts/run_ad_batch.ps1 -Range Last7 -Pack AC

# 本月（JST）+ 指定店铺子目录
scripts/run_ad_batch.ps1 -Range ThisMonth -Pack A -SiteSlug myshop

# 仅打印命令
scripts/run_ad_batch.ps1 -Range Last30 -Pack B -DryRun
```
...
```powershell
scripts/fetch_rpp_search_slices.ps1 -StartDate 2026-03-11 -EndDate 2026-04-09 -SelectionType 2 -ChunkDays 7
```
...
- **路径**：脚本位于 skill 目录下 `scripts/` 子目录；安装后可通过相对路径 `scripts/<脚本名>` 调用。
